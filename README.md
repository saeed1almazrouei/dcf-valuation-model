...
enterprise value under two terminal-value methods — Gordon-growth perpetuity and
a comparable exit multiple. It also builds the free-cash-flow-to-equity (FCFE)
view and reconciles equity value computed directly against equity value backed
out from enterprise value.

The analytical payload is a sensitivity grid showing how enterprise value — and
the *disagreement between the two terminal methods* — respond across a range of
WACC and terminal-growth assumptions. The core logic lives in an importable,
tested `dcf` package; the notebook is only a walkthrough.
...
1. **FCFF** from statement inputs, per period:
   ...
   already captured in WACC via the after-tax cost of debt. ΔNWC is derived from
...
2. **A ten-year projection**, driving every line off revenue: revenue compounds
   at a growth rate (a single rate or a per-year path), and EBIT / D&A / capex /
...
3. **Enterprise value**, discounting the FCFF stream at WACC and adding the
   ...
   - *Exit multiple:* `TV_n = EBITDA_n × exit_multiple`, `EBITDA = EBIT + D&A`.
...
4. **FCFE and a reconciliation check.** FCFE is discounted at the cost of equity
   to give equity value *directly*; that value is compared against
...
Inputs come straight off the statements as dollar figures — no pre-computing
ratios:
...
notebooks/        # walkthrough.ipynb — narrated tour of the whole model
```
...
Running the base case across WACC (7–13%) and terminal growth (0–4%):

- **The two terminal methods disagree by up to ~70%** from identical operating
  assumptions, depending purely on where you sit in the WACC/growth plane.
- **There is an "agreement locus," and it moves.** A diagonal band of near-zero
  divergence marks where the two methods coincide; it slopes from ~0.75%
  terminal growth at 7% WACC to ~3.75% at 10% WACC. Whether your two
  "independent" checks corroborate each other is itself an assumption.
- **At high discount rates the exit multiple looks optimistic.** Above ~11% WACC,
  Gordon growth never reaches an 8× exit in a plausible growth range.
...
**Terminal value dominates the valuation.** In the base case, 56–62% of
...
terminal share climbs toward 70–80%. Most of the answer rests on the part of the
model you can least defend. A DCF is, to a first approximation, a terminal-value
estimate with a decade of decoration in front of it.
...
conveniences, not forecasts. Years 1–3 might reflect genuine visibility; years
4–10 are a smooth extrapolation whose main function is to feed the terminal-value
formula.
...
**Other simplifications.** No mid-year discounting convention (cash is treated as
year-end, slightly understating present value); a flat tax rate with no NOLs or
deferred taxes; and every output is a point estimate — the sensitivity grid
substitutes for, but is not, a probabilistic treatment of the inputs.
